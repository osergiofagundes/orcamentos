...
export function EditProductModal({ isOpen, onClose, product, workspaceId }: EditProductModalProps) {
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || "Erro ao atualizar produto/serviço")
      }
      ...
      toast.error(error instanceof Error ? error.message : "Erro ao atualizar produto/serviço")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg border-l-8 border-l-sky-600 rounded-lg">
        <DialogHeader>
          <DialogTitle className="text-left">Editar Produto/Serviço</DialogTitle>
          <DialogDescription className="text-left">
            Atualize as informações do produto/serviço.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
